...
if(isset($_POST['change-btn'])){
    ...
    $check_password = "SELECT * FROM admin WHERE password = '{$current}' AND id = {$_SESSION['admin_id']}";
    $result = mysqli_query($conn, $check_password);

    if(mysqli_num_rows($result) > 0){
    $update_password = "UPDATE admin SET password = '{$new_password}' WHERE id = {$_SESSION['admin_id']}";
